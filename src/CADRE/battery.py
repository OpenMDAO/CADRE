...
class BatterySOC(rk4.RK4):
    ...
    def __init__(self, n_times, h):
        ...
        self.add_param('P_bat', np.zeros((n_times, )), units="W",
                       desc="Battery power over time")

        self.add_param('temperature', np.zeros((5, n_times )), units="degK",
...
class BatteryPower(Component):
    ...
    def __init__(self, n=2):
        ...
        self.add_param('P_bat', np.zeros((n, )), units="W",
                       desc="Battery power over time")
    ...
    def apply_linear(self, params, unknowns, dparams, dunknowns, dresids, mode):
        ...
        if mode == 'fwd':
            ...
            if 'P_bat' in dparams:
                dI_bat += self.dI_dP * dparams['P_bat']

            if 'temperature' in dparams:
                dI_bat += self.dI_dT * dparams['temperature'][4, :]

            if 'SOC' in dparams:
                dI_bat += self.dI_dSOC * dparams['SOC'][0, :]
...
class BatteryConstraints(Component):
    ...
    def __init__(self, n=2):
        ...
        # Inputs
        self.add_param('I_bat', np.zeros((n, )), units="A",
                       desc="Battery current over time")
    ...
    def solve_nonlinear(self, params, unknowns, resids):
        ...
        unknowns['ConCh'] = self.KS_ch.compute(I_bat - self.Imax, self.rho)
        unknowns['ConDs'] = self.KS_ds.compute(self.Imin - I_bat, self.rho)
    ...
    def apply_linear(self, params, unknowns, dparams, dunknowns, dresids, mode):
        ...
        if mode == 'fwd':
            if 'I_bat' in dparams:
                if 'ConCh' in dresids:
                    dresids['ConCh'] += np.dot(self.dCh_dg, dparams['I_bat'])
                if 'ConDs' in dresids:
                    dresids['ConDs'] -= np.dot(self.dDs_dg, dparams['I_bat'])

            if 'SOC' in dparams:
                if 'ConS0' in dresids:
                    dresids['ConS0'] -= np.dot(self.dS0_dg, dparams['SOC'][0, :])
                if 'ConS1' in dresids:
                    dresids['ConS1'] += np.dot(self.dS1_dg, dparams['SOC'][0, :])

        else:
            if 'I_bat' in dparams:
                dI_bat = dparams['I_bat']
                if 'ConCh' in dresids:
                    dI_bat += self.dCh_dg * dresids['ConCh']
                if 'ConDs' in dresids:
                    dI_bat -= self.dDs_dg * dresids['ConDs']
                dparams['I_bat'] = dI_bat
